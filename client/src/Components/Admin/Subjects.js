...
import SubjectsTableWrapper from './SubjectsTableWrapper';
...
const Subjects = ({ token, session }) => {
  ...
  const handleActionVisible = (courseId, table) => {
    setVisibleActionCardId((prevId) => {
        const newId = prevId === courseId ? null : courseId;
        console.log(newId)
        console.log(table)
        if (table) {
            if (newId) {
                table.classList.add(styles.tableWithExtendedHeight);
            } else {
                table.classList.remove(styles.tableWithExtendedHeight);
            }
        }
        return newId;
    });
  ...
  console.log(subjects, teachers, students, classes)
          ...
          <div className={styles.container}>
            ...
            <div className={styles.topContent}>
              ...
              {isLoading ? (
                ...
                <SubjectsTableWrapper handleSort={handleSort} sortedCourses={sortedCourses} countTotalClasses={countTotalClasses} countTotalSubjects={countTotalSubjects} countTotalTeachers={countTotalTeachers} countTotalStudents={countTotalStudents} toggleDeleteCourse={toggleDeleteCourse} handleActionVisible={handleActionVisible} visibleActionCardId={visibleActionCardId} actionMenuRefs={actionMenuRefs} sortConfig={sortConfig} toggleAddItem={toggleAddItem} teachers={teachers} students={students} classes={classes} subjects={subjects} />
                ) : (
                <div className={`${styles.noData} alert alert-info`}>
                  <h3>No Subjects, Please add one</h3>
                </div>
              )
              )}
            </div>
